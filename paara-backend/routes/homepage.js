...
router.get('/paara-irl',(req,res)=>res.json(db.prepare('SELECT id,image_url,owner_image_url,caption,updated_at FROM paara_irl WHERE id=1').get()||null));
router.get('/worn-by-you',(req,res)=>{ res.set('Cache-Control', 'no-store, no-cache, must-revalidate'); res.json(db.prepare('SELECT id,instagram_post_url,image_url,caption,likes,cached_at FROM instagram_reviews WHERE product_id IS NULL ORDER BY cached_at ASC,id ASC LIMIT 3').all()); });
